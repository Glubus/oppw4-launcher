<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      const data = await apiFetch<{ user: PublicUser; skins: Skin[] }>(`/users/${$page.params.username}`, {}, current?.token);
      user = data.user;
      skins = data.skins;
    } catch (err) {
...
<main class="mx-auto grid max-w-7xl gap-5 px-4 py-6">
        ...
        {#if user.socialLinks.length}
          <div class="flex flex-wrap gap-2">
            {#each user.socialLinks as link}
              <a class="rounded-md border border-border bg-background px-3 py-2 text-sm font-bold text-primary hover:bg-accent" href={link.url} target="_blank" rel="noreferrer">{link.label}</a>
            {/each}
          </div>
        {/if}
      </div>
  ...
  <SkinGrid {skins} {loading} onVote={vote} canManagePins={Boolean(user?.canManagePins)} onTogglePin={togglePin} />
</main>
