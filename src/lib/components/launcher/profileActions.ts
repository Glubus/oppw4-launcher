...
export async function createProfile(ctx: LauncherActionContext, icon = "sparkles", color = "violet") {
  ...
  if (!name) return;
  ctx.logDebug(`creating profile: ${name}`);
...
export async function saveCurrentProfile(ctx: LauncherActionContext, icon = "sparkles", color = "violet") {
  const name = ctx.getProfileName().trim();
  if (!name) return;
  ctx.logDebug(`saving current enabled mods as profile: ${name}`);
  const config = ctx.getConfig();
...
export async function addModToProfile(ctx: LauncherActionContext, profile: ModProfile, mod: InstalledMod) {
  ...
  ctx.logDebug(`adding mod to profile: profile=${profile.name}; mod=${mod.name}`);
  const config = ctx.getConfig();
  ctx.setConfig({
    ...config,
...
export async function applyProfile(ctx: LauncherActionContext, profile: ModProfile) {
  await ctx.runBusy(async () => {
    ctx.logDebug(`applying profile: ${profile.name}; mods=${profile.enabledModKeys.length}`);
    await ctx.save();
...
export async function deleteProfile(ctx: LauncherActionContext, profile: ModProfile) {
  const config = ctx.getConfig();
  ctx.logDebug(`deleting profile: ${profile.name}`);
...
export async function updateProfileStyle(ctx: LauncherActionContext, profile: ModProfile, icon: string, color: string) {
  const config = ctx.getConfig();
  ctx.logDebug(`updating profile style: ${profile.name}; icon=${icon}; color=${color}`);
